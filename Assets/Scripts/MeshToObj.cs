...
public class MeshToObj
{
    ...
    public static void ToObj(MeshFilter mf)
    {
        Mesh meshGo = mf.mesh;
        string datPath = Application.dataPath + "/Assets";
        using (StreamWriter streamWriter = new StreamWriter(string.Format("{0}{1}.obj", datPath, meshGo.name)))
        {
    ...
    private static string MeshToString(MeshFilter mf, Vector3 scale)
    {
        ...
        {
            Vector2 vector2 = mesh.uv[num];//Vector2.Scale(mesh.uv[num], textureScale) + textureOffset;
